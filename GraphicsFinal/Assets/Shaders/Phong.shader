            ...
            Pass {
                ...
                CGPROGRAM
                    ...
                    fixed4 frag(v2f i) : COLOR
                    {
                        ...
                        float3 lightDirection = _WorldSpaceLightPos0.xyz - i.posWorld.xyz * 1;

                        float3 ambientLighting = UNITY_LIGHTMODEL_AMBIENT.rgb * _Color.rgb; //Ambient component
                        float3 diffuseReflection = unity_LightColor[0].rgb * _Color.rgb * max(0.0, dot(normalDirection, lightDirection)); //Diffuse component
                        ...
                        {
                            //Specular component
                            specularReflection = unity_LightColor[0].rgb * _SpecColor.rgb * pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess);
                        }
